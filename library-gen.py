...
class GeminiLibraryGenerator:
    ...
    def generate_main_topics(self, initial_prompt: str, topic_count: int) -> List[str]:
        """Generate specified number of main topics from the initial prompt."""
        print(f"🎯 Generating {topic_count} main topics...")
        
        prompt = f"""
Based on the following request: "{initial_prompt}"

Generate exactly {topic_count} distinct, specific, and comprehensive topics that would be valuable for someone interested in this subject. Each topic should be:
...
Format your response as a numbered list from 1 to {topic_count}, with each topic on a new line.
Example format:
1. Topic Name One
2. Topic Name Two
...
{topic_count}. Topic Name {topic_count}
...
"""
        ...
        if len(topics) < topic_count // 2:  # Fallback if parsing failed
        ...
        return topics[:topic_count]  # Ensure we don't exceed requested count
    
    def generate_subtopics(self, main_topic: str, context: str, subtopic_count: int) -> List[str]:
        """Generate specified number of subtopics for a given main topic."""
        print(f"📚 Generating {subtopic_count} subtopics for: {main_topic}")
        
        prompt = f"""
Context: {context}
Main Topic: "{main_topic}"

Generate exactly {subtopic_count} specific subtopics for this main topic. Each subtopic should be:
...
Format your response as a numbered list from 1 to {subtopic_count}, with each subtopic on a new line.
Example format:
1. Subtopic Name One
2. Subtopic Name Two
...
{subtopic_count}. Subtopic Name {subtopic_count}
...
"""
    ...
    def generate_library(self, initial_prompt: str, topic_count: int, subtopic_count: int, 
                        output_dir: str = "gemini_library"):
        """Generate the complete library with configurable counts."""
        print(f"🚀 Starting library generation for: '{initial_prompt}'")
        print(f"📊 Configuration: {topic_count} main topics × {subtopic_count} subtopics each")
        ...
        main_topics = self.generate_main_topics(initial_prompt, topic_count)
        ...
        total_articles = len(main_topics) * subtopic_count
        ...
        for topic_idx, main_topic in enumerate(main_topics, 1):
            ...
            subtopics = self.generate_subtopics(main_topic, initial_prompt, subtopic_count)
            ...
            for subtopic_idx, subtopic in enumerate(subtopics, 1):
                try:
                    print(f"  📝 Generating article {subtopic_idx}/{subtopic_count}: {subtopic[:50]}...")
...
**Configuration:** {topic_count} main topics × {subtopic_count} subtopics each
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python gemini_library_generator.py --api-key YOUR_KEY --prompt "Create a library for a science lover" --topic-first-count 10 --topic-second-count 20
  python gemini_library_generator.py --api-key YOUR_KEY --prompt "Programming and software development" --topic-first-count 5 --topic-second-count 10 --output-dir my_library
        """
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Number of main topics to generate (default: 100)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="Number of subtopics per main topic (default: 100)"
    )
    ...
    # Validate counts
    if args.topic_first_count <= 0 or args.topic_second_count <= 0:
        print("❌ Topic counts must be positive integers.")
        return 1
    
    if args.topic_first_count > 1000 or args.topic_second_count > 1000:
        print("⚠️ Warning: Very large topic counts may take a very long time and consume significant API quota.")
    ...
    try:
        ...
        generator.generate_library(
            args.prompt, 
            args.topic_first_count, 
            args.topic_second_count, 
            args.output_dir
        )
